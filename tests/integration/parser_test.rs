...
async fn test_parse_xmls_filters_by_target_links() {
    ...
    create_test_xml_file(&subdir1.join("feed.xml"), r#"<?xml version="1.0"?>
<feed>
  <entry><id>1</id><title>One</title></entry>
</feed>"#);

    create_test_xml_file(&subdir2.join("feed.xml"), r#"<?xml version="1.0"?>
<feed>
  <entry><id>2</id><title>Two</title></entry>
    ...
    assert!(temp_dir.path().join("data/parquet/pt/202301.parquet").exists());
    assert!(!temp_dir.path().join("data/parquet/pt/202302.parquet").exists());
}
...
async fn test_parse_xmls_skips_empty_entries() {
    ...
    assert!(!temp_dir.path().join("data/parquet/mc/202301.parquet").exists());
}
...
async fn test_parse_xmls_merges_multiple_files() {
    ...
    create_test_xml_file(&subdir.join("feed1.xml"), r#"<?xml version="1.0"?>
<feed>
  <entry><id>1</id><title>One</title></entry>
</feed>"#);

    create_test_xml_file(&subdir.join("feed2.xml"), r#"<?xml version="1.0"?>
<feed>
  <entry><id>2</id><title>Two</title></entry>
